...
questions = [
    inquirer.List(
        ...
        choices=["Register User", "Logging In User", "Register Card",
                 "Pairing User and Card", "User Request Room", "Admin Give Room Access", "Check In"],
    ),
]
...
# Local TIK #"http://127.0.0.1:80" #Local #for online url please user "https://smart-door-pnj.herokuapp.com"
url_ = "http://127.0.0.1:8000"
secret = {
    "username": "dimasaulia",
    "password": "T4np4$4nd1",
    "api_id": "cl9xxy39x0000cklku2klje55",
    "api_key": "WCPaDR1F2cwSUMPWuXDm13JMYU-NeWO1"
}
buffData = []
RUID = "yuiOB"
DUID = "Hge40"
...
def fetchData(_payload, _endpoint, _no, _worksheet):
    with requests.Session() as client:
        ...
        _worksheet.write(f"D{_no}", round(
...
def fetchDataAdmin(_payload, _endpoint, _no, _worksheet):
    with requests.Session() as client:
        resp = client.post(f'{url_}{_endpoint}',
                           json=_payload, cookies=adminAuth)
        _worksheet.write(f"A{_no}", f"{_no-1}")
        _worksheet.write(f"B{_no}", str(_payload))
        _worksheet.write(f"C{_no}", str(resp.json()))
        _worksheet.write(f"D{_no}", round(
...
def fetchDataAdminAcc(_payload, _endpoint, _no, _worksheet):
    with requests.Session() as client:
        resp = client.post(
            f'{url_}{_endpoint}?ruid={RUID}&cardNumber={_payload["cardNumber"]}', json=_payload, cookies=adminAuth)
        print(resp)
        _worksheet.write(f"A{_no}", f"{_no-1}")
        _worksheet.write(f"B{_no}", str(_payload))
        _worksheet.write(f"C{_no}", str(resp.json()))
...
def fetchDataWCookie(_payload, _endpoint, _no, _worksheet):
    with requests.Session() as client:
        resp = client.post(f'{url_}{_endpoint}?ruid={RUID}&cardNumber={_payload["cardNumber"]}', cookies={
                           "jwt": _payload["jwt"]})
        print(resp)
        _worksheet.write(f"A{_no}", f"{_no-1}")
        _worksheet.write(f"B{_no}", str(_payload))
...
if test.get("test") == "Register User":
    ...
    with ThreadPoolExecutor(threadSize) as executor:
        executor.map(fetchData, jsonData, ["/api/v1/user/register"] * len(
            jsonData), range(2, len(jsonData)+2), [registerWorksheet] * len(jsonData))
...
if test.get("test") == "Logging In User":
    ...
    start = perf_counter()
    with ThreadPoolExecutor(threadSize) as executor:
        executor.map(fetchData, jsonData, ["/api/v1/user/login"] * len(
            jsonData), range(2, len(jsonData)+2), [registerWorksheet] * len(jsonData))
        executor.shutdown(wait=True)
...
if test.get("test") == "Register Card":
    ...
    start = perf_counter()
    with ThreadPoolExecutor(threadSize) as executor:
        executor.map(fetchData, jsonData, [f"/api/v1/card/h/register?id={secret['api_id']}&key={secret['api_key']}"] * len(
            jsonData), range(2, len(jsonData)+2), [registerWorksheet] * len(jsonData))
        executor.shutdown(wait=True)
    print("Execution time:", f"{round((perf_counter() - start),3) * 1000}ms")
...
if test.get("test") == "Pairing User and Card":
    ...
    start = perf_counter()
    with ThreadPoolExecutor(threadSize) as executor:
        executor.map(fetchDataAdmin, jsonData, [f"/api/v1/user/pair"] * len(
            jsonData), range(2, len(jsonData)+2), [registerWorksheet] * len(jsonData))
        executor.shutdown(wait=True)
    print("Execution time:", f"{round((perf_counter() - start),3) * 1000}ms")
...
if test.get("test") == "User Request Room":
    ...
    start = perf_counter()
    with ThreadPoolExecutor(threadSize) as executor:
        executor.map(fetchDataWCookie, jsonData, [f"/api/v1/room/u/request"] * len(
            jsonData), range(2, len(jsonData)+2), [registerWorksheet] * len(jsonData))
        executor.shutdown(wait=True)
    print("Execution time:", f"{round((perf_counter() - start),3) * 1000}ms")
...
if test.get("test") == "Admin Give Room Access":
    ...
    start = perf_counter()
    with ThreadPoolExecutor(threadSize) as executor:
        executor.map(fetchDataAdminAcc, jsonData, [f"/api/v2/room/pair"] * len(
            jsonData), range(2, len(jsonData)+2), [registerWorksheet] * len(jsonData))
        executor.shutdown(wait=True)
    print("Execution time:", f"{round((perf_counter() - start),3) * 1000}ms")
...
if test.get("test") == "Check In":
    ...
    start = perf_counter()
    with ThreadPoolExecutor(threadSize) as executor:
        executor.map(fetchData, jsonData, [f"/api/v2/room/h/check-in/{DUID}?id={secret['api_id']}&key={secret['api_key']}"] * len(
            jsonData), range(2, len(jsonData)+2), [registerWorksheet] * len(jsonData))
        executor.shutdown(wait=True)
    print("Execution time:", f"{round((perf_counter() - start),3) * 1000}ms")
